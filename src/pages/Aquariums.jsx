  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
        <h2 className="font-bold text-lg text-slate-700 mb-4">水槽を追加</h2>
        <div className="flex flex-col gap-3">
          <input className="border rounded-lg px-3 py-2 text-sm" placeholder="水槽名 *" value={name} onChange={e => setName(e.target.value)} />
          <input className="border rounded-lg px-3 py-2 text-sm" type="number" placeholder="容量 (L)" value={volume} onChange={e => setVolume(e.target.value)} />
        </div>
        <div className="flex gap-3 mt-5">
          <button onClick={onClose} className="flex-1 border rounded-lg py-2 text-sm text-slate-500">キャンセル</button>
          <button onClick={() => onSave({ name, volume_liters: volume ? Number(volume) : undefined })} disabled={!name}
            className="flex-1 bg-cyan-600 text-white rounded-lg py-2 text-sm font-medium disabled:opacity-40">追加</button>
        </div>
      </div>
    </div>
  );
...
export default function Aquariums() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {showAdd && <AddModal onClose={() => setShowAdd(false)} onSave={(d) => add.mutate(d)} />}
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {aquariums.map(a => (
            <Link key={a.id} to={`/aquariums/${a.id}`}
              ...
              {a.volume_liters && <p className="text-slate-400 text-sm mt-1">{a.volume_liters}L</p>}
            </Link>
          ))}
        </div>
      )}
